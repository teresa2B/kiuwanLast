...
class ca_customers_search extends ca_customers {
	...
	function __construct() {
		...
		// Start timer
		if (!isset($G_SESSION['gTimer'])) $_SESSION['gTimer'] = new cTimer();
...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
	...
	function Page_Main() {
		...
		if ($this->IsModal)
			$_SESSION['gbSkipHeaderFooter'] = TRUE;
		if ($this->IsPageRequest()) { // Validate request

			// Get action
			$this->CurrentAction = $_SESSION['objForm']->GetValue('a_search');
			switch ($this->CurrentAction) {
				case 'S': // Get search criteria
					...
					if ($this->ValidateSearch()) {
						...
						$sSrchStr = '';
						$this->setFailureMessage($_SESSION['gsSearchError']);
					}
					if ($sSrchStr <> '') {
	...
	function BuildSearchUrl(&$Url, &$Fld, $OprOnly=FALSE) {
		...
		$FldVal = $_SESSION['objForm']->GetValue("x_$FldParm");
		$FldOpr = $_SESSION['objForm']->GetValue("z_$FldParm");
		$FldCond = $_SESSION['objForm']->GetValue("v_$FldParm");
		$FldVal2 = $_SESSION['objForm']->GetValue("y_$FldParm");
		$FldOpr2 = $_SESSION['objForm']->GetValue("w_$FldParm");
	...
	function ValidateSearch() {
		...
		if (!ew_CheckInteger($this->Customer_ID->AdvancedSearch->SearchValue)) {
			ew_AddMessage($_SESSION['gsSearchError'], $this->Customer_ID->FldErrMsg());
		}
	...
	function SetupBreadcrumb() {
		...
		$PageId = 'search';
		$_SESSION['Breadcrumb']->Add('search', $PageId, $url);
	}
