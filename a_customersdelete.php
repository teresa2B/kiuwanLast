...
// End of modification Disable/Enable Registration Page, by Masino Sinaga, May 14, 2012
			...
			case 'D': // Delete
				...
				if ($this->DeleteRows()) { // Delete rows
					if ($this->getSuccessMessage() == '')
						$this->setSuccessMessage($_SESSION['Language']->Phrase('DeleteSuccess')); // Set up success message
	...
	function DeleteRows() {
		...
		if (!$_SESSION['Security']->CanDelete()) {
			$this->setFailureMessage($_SESSION['Language']->Phrase('NoDeletePermission')); // No delete permission
			return FALSE;
		}
		$DeleteRows = TRUE;
		$sSql = $this->SQL();
		...
		if ($rs === FALSE) {
			...
			$this->setFailureMessage($_SESSION['Language']->Phrase('NoRecord')); // No record found
			$rs->Close();
			return FALSE;
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				$sThisKey = '';
				if ($sThisKey <> '') $sThisKey .= $_SESSION['EW_COMPOSITE_KEY_SEPARATOR'];
			...
			if ($this->getSuccessMessage() <> '' || $this->getFailureMessage() <> '') {
				...
				$this->setFailureMessage($_SESSION['Language']->Phrase('DeleteCancelled'));
			}
		}
		if ($DeleteRows) {
